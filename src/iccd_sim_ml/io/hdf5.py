        ...
        for group_key in sorted(handle.keys()):
            ...
            for key in group.keys():
                ...
                if isinstance(obj, h5py.Dataset) and time_ns is not None:
                    if obj.ndim != 2 or obj.shape[1] not in {20, 21}:
                        warnings.append(
                            f"ignored {key}: expected a two-dimensional 20/21-column frame"
                        )
            ...
            times_s = np.asarray(
                [time_ns * 1e-9 for time_ns, _ in pairs],
                dtype=np.float64,
            )
            result.append(
                H5Simulation(
...
def get_h5_simulation(path: str | Path, key: str | None = None) -> H5Simulation:
    ...
    for simulation in simulations:
        if simulation.key == key:
            return simulation
    available = ", ".join(item.key for item in simulations[:10])
